...
class ClickadillaController extends Controller
{
    //--daily_package
    public function daily_package(Request $request)
    {
        $notFreeGames = Game::where('is_free', 0)->get();
        return view('frontend.clickadilla.daily', compact('notFreeGames'));
    }
    //--weekly_package
    public function weekly_package()
    {
        $notFreeGames = Game::where('is_free', 0)->get();
        return view('frontend.clickadilla.weekly', compact('notFreeGames'));
    }
    //--monthly_package
    public function monthly_package()
    {
        $notFreeGames = Game::where('is_free', 0)->get();
        return view('frontend.clickadilla.monthly', compact('notFreeGames'));
    }
    //---promotion_daily
    public function promotion_daily(Request $request)
    {
        // return $request->all();
        $plan = 'daily';
        $click_id = 2344;
        ...
        if ($plan == "daily") {
            ...
            $data = [
                ...
                "urls" => [
                    "ok" => "https://xoss.games/clickadilla/process-subscription?plan=daily&click_id=$click_id",
                    "deny" => "https://xoss.games/deny",
                    "error" => "https://xoss.games/error"
                    // "ok" => "http://127.0.0.1:8000/clickadilla/process-subscription?plan=daily&click_id=$click_id",
    ...
    //--promotion_weekly
    public function promotion_weekly(Request $request)
    {
        $plan = 'weekly';
        $click_id = 2344;
        //---weekly package
        if ($plan == "weekly") {
            ...
            $data = [
                ...
                "stopMessage" => "STOP SUBSCRIPTION",
                "urls" => [
                    "ok" => "https://xoss.games/clickadilla/process-subscription?plan=weekly&click_id=$click_id",
                    "deny" => "https://xoss.games/deny",
                    "error" => "https://xoss.games/error"
                    // "ok" => "http://127.0.0.1:8000/clickadilla/process-subscription?plan=weekly&click_id=$click_id",
                    // "deny" => "http://127.0.0.1:8000/deny",
                    // "error" => "http://127.0.0.1:8000/error"
                ],
                "operatorId" => "GRA-BD",
    ...
    //--promotion_monthly
    public function promotion_monthly(Request $request)
    {
        $plan = 'monthly';
        $click_id = 2344;
        //---monthly package
        if ($plan == "monthly") {
            ...
            $data = [
                ...
                "stopMessage" => "STOP SUBSCRIPTION",
                "urls" => [
                    "ok" => "https://xoss.games/clickadilla/process-subscription?plan=monthly&click_id=$click_id",
                    "deny" => "https://xoss.games/deny",
                    "error" => "https://xoss.games/error"
                    // "ok" => "http://127.0.0.1:8000/clickadilla/process-subscription?plan=monthly&click_id=$click_id",
                    // "deny" => "http://127.0.0.1:8000/deny",
                    // "error" => "http://127.0.0.1:8000/error"
                ],
                "operatorId" => "GRA-BD",
        ...
        if ($response->successful()) {
            ...
            if ($check_acr) {
                ...
                if ($current_timestamp > $subscriber_details->end_time) {
                    ...
                    $purchaseDetails->clickadilla = $request->click_id;
                    //----success message
