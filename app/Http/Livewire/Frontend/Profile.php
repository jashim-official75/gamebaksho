...
class Profile extends Component
{
    ...
    public function update()
    {
        ...
        if ($this->temp_pro_pic) {
            $this->validate([
                'temp_pro_pic' => 'image|max:1024',
            ]);
            if ($this->profile_pic) {
                unlink(public_path('storage/' . $this->profile_pic));
            }
            $profile_pic_path = time() . '-' . $this->temp_pro_pic->getClientOriginalName();
            $this->temp_pro_pic->storeAs('public', $profile_pic_path);
        ...
        $subscriber->update();
        return redirect(request()->header('Referer'))->with('success', 'Profile Updated Successfully !');
    }
    ...
    public function unsubscribe()
    {
        ...
        if (now()->setTimezone("Asia/Dhaka")->timestamp < $subscriptionDetails->end_time) {
            ...
            if ($response->successful()) {
                ...
                return redirect('/')->with('error', 'Ubsubscription failed! Please try again.');
            }
            return redirect('/')->with('error', 'Ubsubscription failed! Please try again.');
        }
        return redirect('/')->with('error', 'Ubsubscription failed! Please try again.');
